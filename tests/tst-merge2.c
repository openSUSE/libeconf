...
main(int argc, char **argv)
{
  ...
  error = econf_get_key_file (&key_file_1, TESTSDIR"tst-merge2-data/etc/tst-merge2.conf", "=", '#');
  if (error || key_file_1 == NULL)
    {
      fprintf (stderr, "ERROR: couldn't read /etc configuration file: %s\n", econf_errString(error));
      return 1;
    }
  error = econf_get_key_file (&key_file_2, TESTSDIR"tst-merge2-data/usr/etc/tst-merge2.conf", "=", '#');
